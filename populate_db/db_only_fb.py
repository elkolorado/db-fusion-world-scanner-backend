...
json_file_path = "cards_urls/fb05.json"
with open(json_file_path, "r") as json_file:
    # Extract filenames from URLs in the JSON
    valid_files = set(os.path.basename(url) for url in json.load(json_file))

# print(valid_files)

# Process only files listed in the JSON
cards_directory = "cards"
for filename in os.listdir(cards_directory):
    if filename.endswith(".webp") and filename in valid_files:  # Check if file is in the JSON list
